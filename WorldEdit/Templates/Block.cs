...
namespace WorldEdit.Templates
{
    ...
    public sealed class Block : ITemplate
    {
        ...
        /// <summary>
        /// Tries to parse the specified string into a <see cref="Block" /> instance. A return value of <c>null</c> indicates
        /// failure.
        /// </summary>
        /// <param name="s">The string to parse, which must not be <c>null</c>.</param>
        /// <returns>The block, or <c>null</c> if parsing failed.</returns>
        ...
        public static Block TryParse([NotNull] string s)
        {
            ...
            if (split.Length == 3)
            {
                var block = TryParse(split[0]);
                if (block == null ||
                    !short.TryParse(split[1], out var frameX) ||
                    !short.TryParse(split[2], out var frameY))
                {
                    return null;
                }
                return new Block(block._type, frameX, frameY);
            }
